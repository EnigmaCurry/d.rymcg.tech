...
## Only clone user supplied template repository if HOMEPAGE_AUTO_CONFIG=false (i.e., if user doesn't want to have d.ry autoconfig Homepage)
if [[ "${HOMEPAGE_AUTO_CONFIG}" == false ]]; then
    ...
    rm -rf /app/config/*
    mv /app/reloader/homepage_config/* /app/config/
