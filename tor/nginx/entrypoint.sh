...
with open('/tmp/nginx.conf', 'w') as f:
    ...
    for name, hostname, listen_port in proxies:
        ...
        f.write(f'            proxy_http_version 1.1;\n')
        f.write(f'            proxy_set_header Upgrade $http_upgrade;\n')
        f.write(f'            proxy_set_header Connection "upgrade";\n')
        f.write(f'            proxy_read_timeout 600s;\n')
