#!/usr/bin/env sh
# ------------------------------------------------------------
#  WireGuard‑router entrypoint with “old‑gateway blackhole”
# ------------------------------------------------------------
...
# -------------------------------------------------------------------------
#  Helper functions (logging, safe iptables calls, etc.)
# -------------------------------------------------------------------------
log() {
...
# Wrapper that tries the legacy binary first, then the nft‑compatible one.
# This works on Alpine, Debian, Ubuntu, etc.  If neither exists the script aborts.
iptables_cmd() {
...
# -------------------------------------------------------------------------
#  1️⃣  Grab the **current** default gateways (if any)
# -------------------------------------------------------------------------
#  IPv4
...
# -------------------------------------------------------------------------
#  2️⃣  Install black‑hole iptables rules for those gateways
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
#  3️⃣  (Optional) sanity‑check – make sure we *did* replace the old gateway
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
#  4️⃣  Drop the privileged capability and exec the original init
# -------------------------------------------------------------------------
