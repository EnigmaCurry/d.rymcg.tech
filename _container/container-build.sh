...
usage() {
    ...
    echo "  --install-extras LIST  Comma-separated CLI tools to install (doctl,aws,gh,rclone)"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --install-extras) INSTALL_EXTRAS="$2"; shift 2 ;;
...
if [[ -n "${INSTALL_EXTRAS}" ]]; then
    BUILD_ARGS+=(--build-arg "INSTALL_EXTRAS=${INSTALL_EXTRAS}")
fi
